<template>
    <Row>
        ...
        <Form ref="groupref" :model="groupForm" :rules='ruleValidate' :label-width="80">
            ...
            <Form-item label="输入框">
                <Cascader :data="groups" v-model="defaultvalue" change-on-select @on-change="selectGroup"></Cascader>
            </Form-item>
            <Form-item>
...
export default {
        ...
        handleSubmit(groupref) {
            ...
            this.$refs[groupref].validate((valid) => {
                if (valid) {
                    ...
                    axios.post('http://localhost:9090/group',this.groupForm)
                    .then(function(res){
    ...
    created: function () {
        ...
        axios.get('http://localhost:9090/group/'+this.$route.params.gid).then(res=>{
          console.info(res);
          this.groupForm = res.data;
          this.defaultvalue.push(res.data.id);
