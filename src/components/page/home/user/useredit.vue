    ...
    <Row>
        ...
        <Form ref="formValidate" :model="user" :rules="ruleValidate" :label-width="80">
            ...
            <Form-item label="组织" prop="group">
                <Select v-model="user.group" placeholder="请选择所在组织">
        ...
        <Col span="14">
        <Tree :data="group" :multiple="false" :show-checkbox="true"></Tree>
        </Col>
    </Row>
</template>
<script>
...
export default {
    ...
    created(){
        if(this.$route.params.uid){
            axios.get('http://localhost:9090/user/'+this.$route.params.uid).then((res)=>{
                this.user = res.data;
            }).catch(function(err){
                console.info(err);
            });
